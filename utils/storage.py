...
def save_to_blob_storage(file_path, blob_name):
    ...
    if account_url is "" or credential is "" or container_name is "":
        raise ValueError(
            "Please set the Azure Document environment variables"
        )
    ...
    with open(file_path, 'rb') as file:
        blob_client.upload_blob(data=file, overwrite=True, content_settings=ContentSettings(content_type='application/pdf'))
